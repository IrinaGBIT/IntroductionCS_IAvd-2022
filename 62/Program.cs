...
// 62 vers 1 - with given variable values
...
void Opposite2DArr(int[,] a, int[,] b)
{
    for (int i = 0; i < a.GetLength(0); i++)
    {
        for (int j = 0; j < a.GetLength(1); j++)
        {
            if ((a[i, j] % 2) == 0) { b[i, j] = -a[i, j]; } else b[i, j] = a[i, j];
        }
    }
}
...
Opposite2DArr(Random2DArray, New2DArray);
Console.WriteLine("Изменённый массив");
Print2DArray(New2DArray);


/*
// 62 vers 2 - manual input value
void Print2DArray(int[,] a)
{
    for (int i = 0; i < a.GetLength(0); i++)
    {
